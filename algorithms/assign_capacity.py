...
def run_capacity_assignment(source_id: str, parameter: int = 1, string_param: str = ""):
    """
    Capacity optimization assignment function - prioritizes maximum utilization
    """
    ...
    progress.start_assignment(source_id, "CAPACITY OPTIMIZATION")
    logger.info(f"🎯 Starting CAPACITY assignment for source_id: {source_id}")

    try:
        # Use the route efficiency assignment as base (from assignment.py)
        # but with capacity-focused parameters
        result = assignment.run_route_efficiency_assignment(source_id, parameter, string_param)

        # Update the optimization mode to reflect capacity approach
        ...
        logger.error(f"Capacity assignment failed: {e}", exc_info=True)
