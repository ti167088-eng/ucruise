...
class ProgressTracker:
    ...
    def start_assignment(self, source_id, mode):
        """Start tracking an assignment"""
        self.start_time = time.time()
        self.source_id = source_id
        self.mode = mode
        self.logger.info(f"📊 Starting {mode} assignment for {source_id}")
    ...
    def start_stage(self, stage_name, description=""):
        ...
        self.logger.info(f"🔄 Stage: {stage_name} - {description}")
    ...
    def complete_stage(self, message=""):
        ...
        if self.current_stage:
            ...
            self.logger.info(f"✅ Completed: {self.current_stage['name']} ({duration:.2f}s) - {message}")
            self.stages.append(self.current_stage)
            self.current_stage = None

    def fail_assignment(self, reason):
        """Log assignment failure"""
        self.logger.error(f"❌ Assignment failed: {reason}")
    ...
    def show_final_summary(self, result):
        ...
        if self.start_time:
            ...
            self.logger.info(f"🚗 Routes created: {routes_count}")
            self.logger.info(f"👥 Unassigned users: {unassigned_count}")
