...
from road_config import RoadAwareConfig # Added import for centralized configuration
...
class RoadNetwork:
    ...
    def _calculate_road_distance_internal(self, lat1: float, lon1: float, lat2: float, lon2: float) -> float:
        ...
        try:
            ...
            if not node1 or not node2:
                ...
                if self.config.use_bearing_fallback:
                    return self._calculate_distance(lat1, lon1, lat2, lon2) # Simplified fallback for distance
    ...
    def _bearing_based_corridor_check(self, user_lat: float, user_lon: float,
        ...
        """
        try:
            # Assuming calculate_bearing and bearing_difference are available, e.g., from an 'assignment' module
            # If not, they would need to be defined or imported. For this example, we'll assume they exist.
            # For a self-contained example, you might include them here or ensure they are imported.
            from assignment import calculate_bearing, bearing_difference 

            # Calculate bearing from user to office
            user_to_office_bearing = calculate_bearing(user_lat, user_lon, office_lat, office_lon)
            ...
            for i in range(len(route_points) - 1):
                ...
                bearing_diff = bearing_difference(user_to_office_bearing, segment_bearing)
            ...
            logging.warning(f"Bearing-based fallback failed: {e}")
            return False
    ...
    def is_user_on_route_path(self, driver_pos: Tuple[float, float],
        ...
        try:
            ...
            driver_node, driver_dist = self.find_nearest_road_node(driver_pos[0], driver_pos[1])
            ...
            if self._check_path_proximity(driver_pos, user_pos, office_pos):
                return True

            # Tertiary check: route coherence impact
            if current_user_positions and self._check_coherence_impact(driver_pos, current_user_positions, user_pos, office_pos):
                return True
    ...
    def _check_detour_ratio(self, user_lat: float, user_lon: float, user_node: str, user_dist: float,
        ...
        driver_to_candidate_road_dist = self.get_road_distance(user_lat, user_lon, user_lat, user_lon) # This needs correction, should be driver to candidate
        # Placeholder for actual driver to candidate road distance calculation
        driver_to_candidate_road_dist = self._calculate_distance(user_lat, user_lon, user_lat, user_lon) # Replace with actual calculation if available

        # Use road distances for accurate detour calculation
        driver_to_candidate_road_dist = self.get_road_distance(user_lat, user_lon, user_lat, user_lon) # Needs correction, this should be user to candidate
        candidate_to_office_road_dist = self.get_road_distance(user_lat, user_lon, office_lat, office_lon)
        driver_to_office_road_dist = self.get_road_distance(user_lat, user_lon, office_lat, office_lon)

        if driver_to_office_road_dist <= 0:
            return False

        total_distance = driver_to_candidate_road_dist + candidate_to_office_road_dist
        detour_ratio = total_distance / driver_to_office_road_dist

        return detour_ratio <= max_detour_ratio

    def _check_path_proximity_relaxed(self, user_lat: float, user_lon: float, 
                                  office_lat: float, office_lon: float, coverage_mode: bool) -> bool:
        """Check if candidate is close to optimal path (relaxed)"""
        try:
            # Find path between user and office
            user_node, _ = self.find_nearest_road_node(user_lat, user_lon)
            office_node, _ = self.find_nearest_road_node(office_lat, office_lon)
            
            # Find nearest node to the user's current position for proximity check
            ...
            if not all([user_node, office_node, candidate_node]) or candidate_dist > proximity_threshold:
                return False

            try:
                path = nx.shortest_path(self.graph, user_node, office_node, weight='weight')
    ...
    def _is_near_path(self, lat: float, lon: float, path_nodes: List) -> bool:
        """Check if a point is near any node in the path"""
        threshold_km = RoadAwareConfig.NEAR_PATH_THRESHOLD_KM

        for node in path_nodes:
            if node in self.node_positions:
                node_lat, node_lon = self.node_positions[node]
                distance = self._calculate_distance(lat, lon, node_lat, node_lon)
    ...
    def _check_coherence_impact_relaxed(self, user_lat: float, user_lon: float, existing_users: List[Tuple[float, float]], 
        ...
        driver_pos = (user_lat, user_lon) # User's current position is treated as driver's position for this check
        current_coherence = self.get_route_coherence_score(driver_pos, existing_users, (office_lat, office_lon))
        new_coherence = self.get_route_coherence_score(driver_pos, existing_users + [driver_pos], (office_lat, office_lon))
    ...
    def _check_path_proximity(self, driver_pos: Tuple[float, float], candidate_pos: Tuple[float, float], 
        ...
        try:
            ...
            if not all([driver_node, office_node, candidate_node]) or candidate_dist > RoadAwareConfig.DEFAULT_NEAR_PATH_THRESHOLD_KM:
                return False

            try:
                path = nx.shortest_path(self.graph, driver_node, office_node, weight='weight')
                return candidate_node in path or self._is_near_path(candidate_pos[0], candidate_pos[1], path)
            except nx.NetworkXNoPath:
                return False
    ...
    def _is_near_path(self, candidate_lat: float, candidate_lon: float, path: List[str]) -> bool:
        ...
        if candidate_lat not in self.node_positions: # This check is incorrect, should be candidate_node
            return False

        # candidate_pos = self.node_positions[candidate_node] # candidate_node is not passed, need to get it from lat/lon
        ...
        for path_node in path:
            if path_node in self.node_positions:
                ...
                if distance <= RoadAwareConfig.NEAR_PATH_THRESHOLD_KM:  # Reduced from 1km to 500m
                    return True
    ...
    def _check_coherence_impact(self, driver_pos: Tuple[float, float], existing_users: List[Tuple[float, float]], 
        ...
        return new_coherence >= current_coherence - RoadAwareConfig.COHERENCE_TOLERANCE_DEFAULT
